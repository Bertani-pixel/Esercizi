...
for valori in range(0,n):
    print("qual'è stata la temperatura massima ragiunta a ", nomi[v], " in °C?")
    mas = float(input())
    max.append(mas)
    print("qual'è stata la temperatura minima ragiunta a ", nomi[v], " in °C?")
    mim = float(input())
    min.append(mim)
    v+=1
...
if c != 0:
    print("l'escursione termica è aumentata in", c, "città; queste città sono:", cambiati)
else:
    breakpoint
